...
energy=np.loadtxt("energy.dat",dtype='float32')
nf=energy.shape[0]
...
force=np.loadtxt("force.dat",dtype='float32')
...
pos=np.loadtxt("pos.dat",dtype='float32')
dataset=pos.reshape(nf,N*3)
np.random.seed(1)
np.random.shuffle(dataset)
data_tr,data_ts = train_test_split(dataset,train_size=0.8,random_state=seed_shuffle,shuffle=True)
np.save("dataset/training/pos.npy",data_tr)
np.save("dataset/test/pos.npy",data_ts)

box=np.loadtxt("box.dat",dtype='float32')
dataset=box.reshape(nf,6)
np.random.seed(1)
np.random.shuffle(dataset)
data_tr,data_ts = train_test_split(dataset,train_size=0.8,random_state=seed_shuffle,shuffle=True)
np.save("dataset/training/box.npy",data_tr)
np.save("dataset/test/box.npy",data_ts)
