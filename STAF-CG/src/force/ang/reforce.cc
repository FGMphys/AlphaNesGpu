...
REGISTER_OP("ComputeForceTripl")
    ...
    void computeforce_tripl_Launcher(const real*  netderiv_T_d, const real* desr_T_d, const real* desa_T_d,
                        ...
                        int nr, int na, int N, int dimbat,int num_finger,const real* type_emb3b_d,
                        const int* actual_type,real* forces3b_T_d,const int *num_triplets_d,
                        const real* smooth_a_T,const int* color_type_map_T_d,
 ...
 public:
  ...
  void Compute(OpKernelContext* context) override {
    ...
    computeforce_tripl_Launcher(netderiv_T_d.data(), desr_T_d.data(), desa_T_d.data(),
                        ...
                        actual_type,forces3b_T->flat<real>().data(),
...
class ComputeForceTriplVirialOp : public OpKernel {
 public:
  ...
  void Compute(OpKernelContext* context) override {
    ...
    const Tensor& num_triplets_T=context->input(12);
    const Tensor& map_intra_T=context->input(13);
    const Tensor& pos_T = context->input(14);
    const Tensor& box_T = context->input(15);
    ...
    computeforce_tripl_virial_Launcher(
        ...
        nr, na, N, dimbat, num_finger, type_emb3b_T.flat<real>().data(),
        actual_type, forces3b_T->flat<real>().data(),
        num_triplets_T.flat<int>().data(), smooth_a_T.flat<real>().data(),
