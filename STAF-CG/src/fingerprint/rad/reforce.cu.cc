...
__global__ void radialAFs_kernel(
        ...
        const int* interaction_map_rad,const real* type_emb2b,const int* color_type_map,
	    ...
	    int nr_particle=intmap_r[b*N_local*(nr+1)+par*(nr+1)];
            if (j<nr_particle)
            {
                ...
                for (int i=0; i<nalpha_r;i++){
                    real alpha_now=alphas[nalpha_r*row_index+i];
                    real chpar=type_emb2b[nalpha_r*row_index+i];
