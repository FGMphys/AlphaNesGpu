...
REGISTER_OP("ComputeForceTriplGrad")
    ...
    void gradforce_tripl_Launcher(const real*  prevgrad_T_d,const real*  netderiv_T_d, const real* desr_T_d,
                                      ...
                                      int dimbat,int num_finger,const real* type_emb3b_d,
                                      ...
                                      real* gradnet_3b_T_d,real* grad_alpha3b_T_d,real* grad_emb3b_T_d);
...
class ComputeForceTriplGradOp : public OpKernel {
 public:
  ...
  void Compute(OpKernelContext* context) override {
    ...
    const Tensor& color_type_map_T = context->input(10);
...
//   gradnet_3b_shape.AddDim (1);
    ...
    gradforce_tripl_Launcher(prevgrad_T_d.data(),netderiv_T_d.data(), desr_T_d.data(), desa_T_d.data(),intderiv_r_T_d.data(),intderiv_a_T_d.data(),
                        ...
                        color_type_map_T_d.data(),prod,map_intra_T_d.data(),gradnet_3b_T->flat<real>().data(),
...
void gradforce_tripl_virial_Launcher(const real*  prevgrad_T_d,const real* prevgrad_virial,
...
class ComputeForceTriplVirialGradOp : public OpKernel {
 public:
  ...
  void Compute(OpKernelContext* context) override {
    ...
    const Tensor& color_type_map_T = context->input(11);
    const Tensor& map_color_interaction_T = context->input(12);
    const Tensor& actual_type_T = context->input(13);
    const Tensor& num_triplets_T=context->input(14);
    const Tensor& map_intra_T=context->input(15);
    const Tensor& pos_T = context->input(16);
    const Tensor& box_T = context->input(17);
    ...
    set_tensor_to_zero_real(grad_alpha3b_T->flat<real>().data(),nt_couple*3*num_finger);
    ...
    gradforce_tripl_virial_Launcher(
                        ...
                        map_color_interaction_T_d.data(),
                        actual_type,num_triplets_T_d.data(),smooth_a_T_d.data(),
                        color_type_map_T_d.data(),prod,map_intra_T_d.data(),
                        pos_T.flat<real>().data(),box_T.flat<real>().data(),
                        gradnet_3b_T->flat<real>().data(),
                        grad_alpha3b_T->flat<real>().data(),
  			grad_emb3b_T->flat<real>().data());
}
};
