...
__global__ void DescriptorsRadial_kernel(real range,int radial_buffer,real range_angolare,int angular_buffer,int N,
                      ...
                      real* der3bsupp, int nf,int* numtriplet,
		                  real rs, real coeffa_intra,real coeffb_intra,real coeffc_intra,real coeffa_inter,real coeffb_inter
                      ,real coeffc_inter,real pow_alpha, real pow_beta,
          real Rs_inter, real Rc_inter, const int* map_intra_d,real Ra_inter)
{
  ...
  if (t<nf*N*radial_buffer)
  {
    ...
    if (k<howmany[b*N+i])
    {
      ...
      dist_norm=staf_sqrt(SQR(dist.x)+SQR(dist.y)+SQR(dist.z));
      ...
      if (same==0){
        ...
        if (dist_norm<rs){
            descriptors[actual_pos+k]=coeffa_intra/staf_pow(dist_norm/rs,pow_alpha)+coeffb_intra/staf_pow(dist_norm/rs,pow_beta)+coeffc_intra;

            real der_cutoff=(-pow_alpha*coeffa_intra/staf_pow(dist_norm/rs,pow_alpha+1.)/rs-pow_beta*coeffb_intra/staf_pow(dist_norm/rs,pow_beta+1.)/rs);
      ...
      {
        des3bsupp[actual_pos+k]=0.5*(staf_cos(PI*dist_norm/Ra_inter)+1);
        der3bsupp[b*N*3*radial_buffer+i*3*radial_buffer+k]=-0.5*staf_sin(PI*dist_norm/Ra_inter)*PI/Ra_inter*dist.x/dist_norm;
        der3bsupp[b*N*3*radial_buffer+i*3*radial_buffer+radial_buffer+k]=-0.5*staf_sin(PI*dist_norm/Ra_inter)*PI/Ra_inter*dist.y/dist_norm;
        der3bsupp[b*N*3*radial_buffer+i*3*radial_buffer+radial_buffer*2+k]=-0.5*staf_sin(PI*dist_norm/Ra_inter)*PI/Ra_inter*dist.z/dist_norm;
      }

      if (dist_norm<Rs_inter){
            descriptors[actual_pos+k]=coeffa_inter/staf_pow(dist_norm/Rs_inter,pow_alpha)+coeffb_inter/staf_pow(dist_norm/Rs_inter,pow_beta)+coeffc_inter;

            real der_cutoff=(-pow_alpha*coeffa_inter/staf_pow(dist_norm/Rs_inter,pow_alpha+1.)/Rs_inter-pow_beta*coeffb_inter/staf_pow(dist_norm/Rs_inter,pow_beta+1.)/Rs_inter);
      ...
      descriptors[actual_pos+k]=0.5*(staf_cos(PI*dist_norm/Rc_inter)+1);
      der2b[b*N*3*radial_buffer+i*3*radial_buffer+k]=-0.5*staf_sin(PI*dist_norm/Rc_inter)*PI/Rc_inter*dist.x/dist_norm;
      der2b[b*N*3*radial_buffer+i*3*radial_buffer+radial_buffer+k]=-0.5*staf_sin(PI*dist_norm/Rc_inter)*PI/Rc_inter*dist.y/dist_norm;
      der2b[b*N*3*radial_buffer+i*3*radial_buffer+radial_buffer*2+k]=-0.5*staf_sin(PI*dist_norm/Rc_inter)*PI/Rc_inter*dist.z/dist_norm;
        }
...
__global__ void DescriptorsAngular_kernel(real range,int radial_buffer,real range_angolare,int angular_buffer,int N,
                      ...
                      real* der3bsupp, int nf,int* numtriplet,const int* map_intra_d,real Ra_inter)
{
  int t=blockIdx.x*blockDim.x+threadIdx.x;
  ...
  if (t<nf*N*angular_buffer)
  {
    ...
    dist_normj=staf_sqrt(SQR(distj.x)+SQR(distj.y)+SQR(distj.z));
    ...
    dist_normk=staf_sqrt(SQR(distk.x)+SQR(distk.y)+SQR(distk.z));
    ...
    real range_angolare_j,range_angolare_k;
  ...
  //QUESTO IF NON SERVE GIA CONTROLLO QUANDO CREA NUMTRIPLET
    if (dist_normj<range_angolare_j && dist_normk<range_angolare_k){
        ...
        cutoffj=real(0.5)*(real(1.)+staf_cos(PI*dist_normj/range_angolare_j));
        cutoffk=real(0.5)*(real(1.)+staf_cos(PI*dist_normk/range_angolare_k));

        real tijk=0.5*(angle+1)*cutoffj*cutoffk;


        dcij.x=-real(0.5)*staf_sin(PI*dist_normj/range_angolare_j)*PI/range_angolare_j/dist_normj*distj.x;
        dcij.y=-real(0.5)*staf_sin(PI*dist_normj/range_angolare_j)*PI/range_angolare_j/dist_normj*distj.y;
        dcij.z=-real(0.5)*staf_sin(PI*dist_normj/range_angolare_j)*PI/range_angolare_j/dist_normj*distj.z;

        dcik.x=-real(0.5)*staf_sin(PI*dist_normk/range_angolare_k)*PI/range_angolare_k/dist_normk*distk.x;
        dcik.y=-real(0.5)*staf_sin(PI*dist_normk/range_angolare_k)*PI/range_angolare_k/dist_normk*distk.y;
        dcik.z=-real(0.5)*staf_sin(PI*dist_normk/range_angolare_k)*PI/range_angolare_k/dist_normk*distk.z;
...
void fill_radial_launcher(real R_c,int radbuff,real R_a,int angbuff,int N,
                      ...
                      real pow_alpha, real pow_beta,real Rs_inter,real Rc_inter,
