...
listcell* getList(const real Box[],real cutoff,int num_particles)
{
	...
	l->NumberCells_x=(int)(volume/(staf_sqrt(Box[3]*Box[3]*Box[5]*Box[5]+Box[5]*Box[5]*Box[1]*Box[1]+Box[3]*Box[3]*Box[2]*Box[2]+Box[4]*Box[4]*Box[1]*Box[1])*cutoff));
	l->NumberCells_y=(int)(volume/(Box[0]*staf_sqrt(Box[5]*Box[5]+Box[4]*Box[4])*cutoff));
