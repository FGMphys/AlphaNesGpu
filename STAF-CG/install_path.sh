#!/bin/sh
# Compile STAF-CG custom ops from the single source tree STAF-CG/src
# into precision-specific output trees ops_{float,double}/src.
#
# Usage: ./install_path.sh [float|double|all]
# Default: all (Sprint 2 dual precision)

set -e
PRECISION="${1:-all}"
STAF_CG_HOME="$(cd "$(dirname "$0")" && pwd)"
REPO_ROOT="$(cd "$STAF_CG_HOME/.." && pwd)"
STAF_CG_SRC="$STAF_CG_HOME/src"
export STAF_INC="${STAF_INC:-$REPO_ROOT/STAF/include}"

NVCC_PATH="${STAF_CG_NVCC:-${STAF_NVCC:-${NVCC:-}}}"
...
if [ -z "$NVCC_PATH" ] || [ ! -x "$NVCC_PATH" ]; then
  echo "STAF-CG: nvcc not found; set STAF_CG_NVCC or install CUDA" >&2
  exit 1
fi

CUDA_HOME="$(cd "$(dirname "$NVCC_PATH")/.." && pwd)"
GPP_PATH="${STAF_CG_GPP:-${STAF_GPP:-/usr/bin/g++}}"
CUDA_LIB64_PATH="${STAF_CG_CUDA_LIB64:-${STAF_CUDA_LIB64:-$CUDA_HOME/lib64}}"
CUDA_INCLUDE_PATH="${STAF_CG_CUDA_INCLUDE:-${STAF_CUDA_INCLUDE:-$CUDA_HOME/include}}"
...
COMPCAP=$("$PYTHON_PATH" "$STAF_CG_HOME/get_compcap.py")
case "$COMPCAP" in
  sm_*) ;;
  *)
    COMPCAP="${STAF_CG_COMPCAP:-sm_70}"
    echo "STAF-CG: get_compcap.py did not return sm_*; using $COMPCAP"
    ;;
esac
...
compile_ops() {
  prec="$1"   # float | double
  out_dir="$STAF_CG_HOME/ops_${prec}"
  echo "==== Compiling $prec → $out_dir (from $STAF_CG_SRC) ===="
...
echo "STAF-CG: install_path done ($PRECISION)"
