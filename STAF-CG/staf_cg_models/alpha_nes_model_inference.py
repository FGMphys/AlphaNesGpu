...
class alpha_nes_full_inference(tf.Module):
      def __init__(self,modelname):
          ...
          dt=np_dtype()
          init_alpha2b=[np.loadtxt(modelname+'/type'+str(k)+'_alpha_2body.dat',dtype=dt).reshape((3,-1)) for k in range(self.number_of_NN)]
          init_alpha3b=[np.loadtxt(modelname+'/type'+str(k)+'_alpha_3body.dat',dtype=dt).reshape((6,-1)) for k in range(self.number_of_NN)]

          initial_type_emb2b=[np.loadtxt(modelname+'/type'+str(k)+'_type_emb_2b.dat',dtype=dt) for k in range(self.number_of_NN)]
          initial_type_emb3b=[np.loadtxt(modelname+'/type'+str(k)+'_type_emb_3b.dat',dtype=dt) for k in range(self.number_of_NN)]
