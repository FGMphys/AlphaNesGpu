...
class ComputeSortProj3bodyOp : public OpKernel {
 public:
  ...
  void Compute(OpKernelContext* context) override {
    ...
    auto alpha3b_parameters = alpha3b_parameters_T.flat<float>();
    ...
    int nsmooth_a=alpha3b_parameters_T.shape().dim_size(1);
    ...
    OP_REQUIRES_OK(context, context->allocate_output(0, angular_AFs_shape,
                                                     &angular_AFs_T));
    set_tensor_to_zero_float(angular_AFs_T->flat<float>().data(),dimbat*Nlocal*nsmooth_a);

    //Computing three-body atomic-fingerprints
    angularAFs_Launcher(radial_descriptor.data(),angular_descriptor.data(),nr,na,
                          angular_AFs_T->flat<float>().data(),dimbat,Nlocal,interaction_map_angular.data(),
