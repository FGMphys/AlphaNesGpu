...
/* When set (DD + LAMMPS ghosts), skip rint PBC — ghosts already carry images. */
__device__ __constant__ int staf_desc_skip_pbc = 0;

extern "C" void staf_descriptor_set_skip_pbc(int skip)
{
  cudaMemcpyToSymbol(staf_desc_skip_pbc, &skip, sizeof(int));
}
...
__global__ void DescriptorsRadial_kernel(double range,int radial_buffer,double range_angolare,int angular_buffer,int N,int N_all,
		      ...
		      double rs, double coeffa,double coeffb,double coeffc,double pow_alpha, double pow_beta)
{
  ...
  if (t<nf*N*radial_buffer)
  {
    ...
    if (k<howmany[b*N+i])
    {
      ...
      olddist.x=coor[b*N_all+i].x-coor[b*N_all+j].x;
      olddist.y=coor[b*N_all+i].y-coor[b*N_all+j].y;
      olddist.z=coor[b*N_all+i].z-coor[b*N_all+j].z;

      if (!staf_desc_skip_pbc) {
...
__global__ void DescriptorsAngular_kernel(double range,int radial_buffer,double range_angolare,int angular_buffer,int N,int N_all,
                      double* position,const double* boxes,
                      int *howmany,int *with,
                      ...
                      double* der3bsupp, int nf,int* numtriplet)
{
  ...
  if (t<nf*N*angular_buffer)
  {
    ...
    olddist.x=coor[b*N_all+i].x-coor[b*N_all+j_who].x;
    olddist.y=coor[b*N_all+i].y-coor[b*N_all+j_who].y;
    olddist.z=coor[b*N_all+i].z-coor[b*N_all+j_who].z;
    ...
    olddist.x=coor[b*N_all+i].x-coor[b*N_all+k_who].x;
    olddist.y=coor[b*N_all+i].y-coor[b*N_all+k_who].y;
    olddist.z=coor[b*N_all+i].z-coor[b*N_all+k_who].z;
...
void fill_radial_launcher(double R_c,int radbuff,double R_a,int angbuff,int N,int N_all,
      ...
      DescriptorsRadial_kernel<<<dimGrid, dimBlock,
                  0, nullptr>>>(R_c,radbuff,R_a,angbuff,N,N_all,inopos_d,box_d,
...
void fill_angular_launcher(double R_c,int radbuff,double R_a,int angbuff,int N,int N_all,
                ...
                DescriptorsAngular_kernel<<<dimGrid,dimBlock,0, nullptr>>>(R_c,radbuff,R_a,angbuff,N,N_all,
