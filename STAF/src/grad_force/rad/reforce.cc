 ...
 public:
  ...
  void Compute(OpKernelContext* context) override {
    const cudaStream_t stream = context->eigen_device<Eigen::GpuDevice>().stream();
      ...
      const Tensor& type_map_T = context->input(8);
      const Tensor& tipos_T = context->input(9);
      const Tensor& actual_type_T = context->input(10);
      const Tensor& pos_T = context->input(11);
      const Tensor& box_T = context->input(12);
