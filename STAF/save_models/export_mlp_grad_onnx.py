#!/usr/bin/env python3
"""Export STAF MLP ONNX with autodiff ∂sum(atomic)/∂af for inference ORT.
...
def main() -> int:
    ...
    if nt == 0:
        ...
        return 1
    print(f"STAF: grad-export {nt} type(s) → {out_dir}")
    ...
    with open(os.path.join(out_dir, "EXPORT_ONNX.txt"), "w", encoding="utf-8") as fh:
        fh.write("STAF MLP grad export (inference-ORT friendly)\n")
