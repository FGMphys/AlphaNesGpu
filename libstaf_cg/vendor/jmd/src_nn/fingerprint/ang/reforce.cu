...
__global__ void angularAFs_kernel(const double* radial_descriptor,const double* angular_descriptor,
                          ...
                          int nsmooth_a,int nsmooth_r,const double* type_emb3b,
                          const int* color_type_map,const int* num_triplets,const int* map_intra,const int* map_color_interaction)
{
    ...
    if (t<N_local*dimbat*na)
    {
        ...
        for (int a1=0;a1<nsmooth_a;a1++){
             ...
             //Ho aggiunto il salto delle radiali
             atomicAdd((double*)&three_body_AFs[b*(nsmooth_a+nsmooth_r)*N_local+par*(nsmooth_a+nsmooth_r)+nsmooth_r+a1],angulardes*softmaxweight*chtjy_par/2.f);
            }
              }
      }
...
void angularAFs_Launcher(const double* radial_descriptor,const double* angular_descriptor,int nr,int na,
                          ...
                          angularAFs_kernel<<<dimGrid, dimBlock, 0, nullptr>>>(radial_descriptor,angular_descriptor,               nr,na,three_body_AFs,dimbat,N_local,
                           interaction_map_angular,alpha3b_parameters,
                          nsmooth_a,nsmooth_r,type_emb3b,
                          color_type_map,num_triplets,map_intra,map_color_interaction);
