...
__global__ void esplora_gpu_data_kernel(double* data,double* data2,double* data3,int* data4,int dim){
...
void esplora_gpu_data_launcher(double* data,double* data2,double* data3,int* data4,int dim){
     int grids=ceil(double(dim)/double(300));
     dim3 dimGrid(grids,1,1);
     dim3 dimBlock(300,1,1);
     esplora_gpu_data_kernel<<<dimGrid,dimBlock,0,nullptr>>>(data,data2,data3,data4,dim);
     cudaDeviceSynchronize();
}



__global__ void print_gpu_data_kernel(double* data,int dim){
        int t=blockIdx.x*blockDim.x+threadIdx.x;
        if (t<dim){
         printf("%d %lf\n",t,data[t]);
   	}
...
void print_gpu_data_launcher(double* data,int dim){
     ...
     dim3 dimGrid(grids,1,1);
     dim3 dimBlock(300,1,1);
     print_gpu_data_kernel<<<dimGrid,dimBlock,0,nullptr>>>(data,dim);
     cudaDeviceSynchronize();
}


__global__ void print_gpu_data_int_kernel(int* data,int dim){
        int t=blockIdx.x*blockDim.x+threadIdx.x;
        if (t<dim){
         printf("%d %d\n",t,data[t]);
     }
...
void print_gpu_data_int_launcher(int* data,int dim){
     ...
     dim3 dimGrid(grids,1,1);
     dim3 dimBlock(300,1,1);
     print_gpu_data_int_kernel<<<dimGrid,dimBlock,0,nullptr>>>(data,dim);
     cudaDeviceSynchronize();
}
__global__ void DescriptorsRadial_kernel(double range_intra,int radial_buffer,double range_angolare_intra,int angular_buffer,int N,
		      ...
		      double rs_intra, double coeffa_intra,double coeffb_intra,double coeffc_intra,double coeffa_inter,double coeffb_inter,double coeffc_inter,double pow_alpha, double pow_beta,
          double Rc_inter,double Rs_inter,double Ra_inter,int* map_intra_d,int* color_type_map_d)
{
  int t=blockIdx.x*blockDim.x+threadIdx.x;
  // from t to b,par,j,k
  ...
  if (t<nf*N*radial_buffer)
  {
    ...
    if (k<howmany[b*N+i])
    {
      ...
      int col_par=color_type_map_d[i];
      int col_j=color_type_map_d[j];

      int mol_i=map_intra_d[i];
      int mol_j=map_intra_d[j];

      int chtype=mol_i-mol_j;
      double range_angolare,rs,range;
      double coeffa,coeffb,coeffc;
      if (chtype==0){
          range_angolare=range_angolare_intra;
          rs=rs_intra;
	  coeffa=coeffa_intra;
          coeffb=coeffb_intra;
	  coeffc=coeffc_intra;
      ...
      if (dist_norm<range_angolare_intra)
...
//        num_angolare[threadIdx.x].z=1;
      ...
      if (dist_norm<rs){
          ...
          der2b[b*N*3*radial_buffer+i*3*radial_buffer+k]=(-pow_alpha*coeffa/pow(dist_norm/rs,pow_alpha+1.)-pow_beta*coeffb/pow(dist_norm/rs,pow_beta+1.))/rs*dist.x/dist_norm;
          der2b[b*N*3*radial_buffer+i*3*radial_buffer+radial_buffer+k]=(-pow_alpha*coeffa/pow(dist_norm/rs,pow_alpha+1.)-pow_beta*coeffb/pow(dist_norm/rs,pow_beta+1.))/rs*dist.y/dist_norm;
          der2b[b*N*3*radial_buffer+i*3*radial_buffer+radial_buffer*2+k]=(-pow_alpha*coeffa/pow(dist_norm/rs,pow_alpha+1.)-pow_beta*coeffb/pow(dist_norm/rs,pow_beta+1.))/rs*dist.z/dist_norm;
      }
      else if (dist_norm<range){
...
__global__ void DescriptorsAngular_kernel(double range_intra,int radial_buffer,double range_angolare_intra,int angular_buffer,int N,
                      double* position,const double* boxes,
                      int *howmany,int *with,
                      double* descriptors,int* intmap3b_l,
                      double* des3bsupp,double* der3b_l,
                      double* der3bsupp, int nf,int* numtriplet,double Rc_inter,double Rs_inter,double Ra_inter,
                      int* map_intra_d,int* color_type_map_d)
{
  int t=blockIdx.x*blockDim.x+threadIdx.x;
  ...
  if (t<nf*N*angular_buffer)
  {
    ...
    int mol_i=map_intra_d[i];
    int mol_j=map_intra_d[j_who];
    int mol_k=map_intra_d[k_who];

    int chtype_j=mol_i-mol_j;
    int chtype_k=mol_i-mol_k;

    double range_angolare_j,rs_j,range_j,range_angolare_k,rs_k,range_k;
    if (chtype_j==0){
        range_angolare_j=range_angolare_intra;
        range_j=range_intra;
    }
    else{
    ...
    if (chtype_k==0){
        range_angolare_k=range_angolare_intra;
        range_k=range_intra;
    }
    else{
    ...
    if (dist_normj<range_angolare_j && dist_normk<range_angolare_k){
        ...
        cutoffj=0.5f*(1.f+cosf(PI*dist_normj/range_angolare_j));
        dcij.x=-0.5f*sinf(PI*dist_normj/range_angolare_j)*PI/range_angolare_j/dist_normj*distj.x;
        dcij.y=-0.5f*sinf(PI*dist_normj/range_angolare_j)*PI/range_angolare_j/dist_normj*distj.y;
        dcij.z=-0.5f*sinf(PI*dist_normj/range_angolare_j)*PI/range_angolare_j/dist_normj*distj.z;

        cutoffk=0.5f*(1.f+cosf(PI*dist_normk/range_angolare_k));
        dcik.x=-0.5f*sinf(PI*dist_normk/range_angolare_k)*PI/range_angolare_k/dist_normk*distk.x;
        dcik.y=-0.5f*sinf(PI*dist_normk/range_angolare_k)*PI/range_angolare_k/dist_normk*distk.y;
        dcik.z=-0.5f*sinf(PI*dist_normk/range_angolare_k)*PI/range_angolare_k/dist_normk*distk.z;
...
void fill_radial_launcher(double R_c,int radbuff,double R_a,int angbuff,int N,
                      ...
                      double rs, double coeffa_intra,double coeffb_intra,double coeffc_intra,double coeffa_inter,double coeffb_inter,double coeffc_inter,double pow_alpha, 
                      double pow_beta, double Rc_inter,double Rs_inter,double Ra_inter,
                      int* map_intra_d,int* type_map_color_d){
      dim3 dimGrid(ceil(double(nf*N*radbuff)/double(BLOCK_DIM)),1,1);
      dim3 dimBlock(BLOCK_DIM,1,1);
      set_tensor_to_zero_int(numtriplet_d,N);
      set_tensor_to_zero_double(descriptor_d,N*radbuff);
      set_tensor_to_zero_double(der2b_d,N*3*radbuff);
      set_tensor_to_zero_double(des3bsupp_d,N*radbuff);
      set_tensor_to_zero_double(der3bsupp_d,N*3*radbuff);
      set_tensor_to_zero_int(intmap2b_d,N*(radbuff+1));
      DescriptorsRadial_kernel<<<dimGrid, dimBlock,
                  0, nullptr>>>(R_c,radbuff,R_a,angbuff,N,inopos_d,box_d,
                      ...
                      rs,coeffa_intra,coeffb_intra,coeffc_intra,coeffa_inter,coeffb_inter,coeffc_inter,pow_alpha,pow_beta,
                      Rc_inter, Rs_inter, Ra_inter,map_intra_d,type_map_color_d);
      cudaDeviceSynchronize();
...
void fill_angular_launcher(double R_c,int radbuff,double R_a,int angbuff,int N,
                      ...
                      double* der3bsupp_d, int nf,int* numtriplet_d,
                      double Rc_inter,double Rs_inter,double Ra_inter,
                      int* map_intra_d,int* type_map_color_d){

                dim3 dimGrid(ceil(double(nf*N*angbuff)/double(BLOCK_DIM)),1,1);
                dim3 dimBlock(BLOCK_DIM,1,1);

		set_tensor_to_zero_double(ang_descr_d,N*angbuff);
		set_tensor_to_zero_int(intmap3b_d,N*2*angbuff);
		set_tensor_to_zero_double(der3b_d,N*angbuff*2*3);
                DescriptorsAngular_kernel<<<dimGrid,dimBlock,0, nullptr>>>(R_c,radbuff,R_a,angbuff,N,
                           inopos_d,box_d,howmany_d,with_d,
                           ang_descr_d,intmap3b_d,des3bsupp_d,der3b_d,
                           der3bsupp_d,nf,numtriplet_d,Rc_inter, Rs_inter, Ra_inter,
                           map_intra_d,type_map_color_d);
...
void set_tensor_to_zero_double(double* tensor,int dimten){
     ...
     dim3 dimGrid(grids,1,1);
     dim3 dimBlock(300,1,1);
     set_tensor_to_zero_double_kernel<<<dimGrid,dimBlock, 0, nullptr>>>(tensor,dimten);
     cudaDeviceSynchronize();
     }
...
void set_tensor_to_zero_int(int* tensor,int dimten){
     ...
     dim3 dimGrid(grids,1,1);
     dim3 dimBlock(300,1,1);
     set_tensor_to_zero_int_kernel<<<dimGrid,dimBlock, 0, nullptr>>>(tensor,dimten);
     cudaDeviceSynchronize();
     }
...
void check_max_launcher(int* tensor,int dimten,int maxval,int* resval){
     ...
     dim3 dimGrid(grids,1,1);
     dim3 dimBlock(300,1,1);
     check_max_kernel<<<dimGrid,dimBlock, 0, nullptr>>>(tensor,dimten,maxval,resval);
     cudaDeviceSynchronize();
}
